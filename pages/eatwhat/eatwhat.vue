...
<view id="tempContainer">
    <view v-for="(item, index) in tempFoods" :key="index" @animationend="handleAnimationEnd" class="tempItem" :data-id="item.id" :style="'top:' + item.top + ';left:' + item.left + ';font-size:' + item.fontSize + ';color:' + item.color">
    {{item.food}}
  </view>
</view>
<view id="outer">
    <view id="inner" :style="'margin-top:' + navbarHeight">
        <view :class="status + ' ' + (restaurants.length?'restaurants':'')" id="main">
            ...
            <view :animation="titleAnimationData" @tap="changeTime" class="title">今天<text class="time">{{menu[menuIdx][0]}}</text>吃<text class="what">{{food}}</text>
...
<!--    <view class="adContainer">
        <ad unitId="adunit-f775b490db25dd6c"></ad>
    </view> -->
    <view class="safeAreaInsetBottom"></view>
</view>
<view id="menuModal" v-if="showMenu">
    <form @submit="handleFormSubmit" :style="'margin-top:' + navbarHeight">
...
var a = new (require("../../libs/qqmap-wx-jssdk.min.js"))({
  key: "XUUBZ-UUCEO-SNAWD-SPT5J-Y2FMF-FNBCY" //换成自己的百度地理位置key
}),
    e = new Date().getHours(),
    n = uni.getSystemInfoSync(),
    i = uni.getMenuButtonBoundingClientRect();

export default {
  data() {
    return {
      navbarHeight: n.statusBarHeight + i.height + 2 * (i.top - n.statusBarHeight) + "px",
    ...
    play: function () {
      ...
      if ("running" === this.status) clearTimeout(this.timer), this.setData({
        ...
        !function n() {
          ...
          }), t(i, "food", s), t(i, "btnText", "停"), i)), e++, a.timer = setTimeout(n, 60);
        }();
      }
    },
